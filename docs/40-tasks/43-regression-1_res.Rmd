...
```{r}
gf_point(Net_Positive ~ Unemployment_Rate_Rounded, data = bj) %>% 
  gf_lm()
```
...
m2 <- lm(Net_Positive ~ Unemployment_Rate_Rounded, data = bj)
summary(m2)
```
...
```{r}
gf_point(Net_Positive ~ Lockdown_Indicator, data = bj) %>% 
  gf_lm()
```

```{r}
## Does lockdown status affect popularity?
m3 <- lm(Net_Positive ~ Lockdown_Indicator, data = bj)
summary(m3)
```
...
Using data from Osterman (2021) "Can We Trust Education for Fostering Trust? Quasi-experimental Evidence on the Effect of Education and Tracking on Social Trust"
...
knitr::kable(data.frame(t(sumtab)), caption = "Table 2 in Ãsterman (2021)")
```
...
```{r}
gf_point(trustindex3 ~ eduyrs25, data = data) %>%
  gf_lm()
```
...
```{r}
cor(trustindex3 ~ eduyrs25, data = data, use = "complete.obs")
```

## 3.3. Regress 'trustindex3' on 'eduyrs25' and interpret the results

```{r}
m4 <- lm(trustindex3 ~ eduyrs25, data = data)
summary(m4)
```


## 3.4. Do the above steps for 'trustindex3' and 'agea'


```{r}
gf_point(trustindex3 ~ agea, data = data) %>%
  gf_lm()
```


```{r}
cor(trustindex3 ~ agea, data = data, use = "complete.obs")
```


```{r}
m5 <- lm(trustindex3 ~ agea, data = data)
summary(m5)
```


## 3.5. Do the above steps for 'trustindex3' and 'female'


```{r}
gf_point(trustindex3 ~ female, data = data) %>%
  gf_lm()
```


```{r}
cor(trustindex3 ~ female, data = data, use = "complete.obs")
```


```{r}
m6 <- lm(trustindex3 ~ female, data = data)
summary(m6)
```
