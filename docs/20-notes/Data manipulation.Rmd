...
knitr::opts_chunk$set(fig.width=6, fig.asp = 0.618, warning=FALSE, comment=NA, prompt=FALSE, message = F, strip.white = F, class.source = "bc-b", attr.source='.numberLines')
```
...
The `sjmisc::rec()` function from `{sjmisc}` allows specifying in detail how values should be recoded. These are many options apropriate for different purposes, so [read more about the function here](https://rdrr.io/cran/sjmisc/man/rec.html). In this example, let's recode the two 'religiosity' variables so that we collapse their 7 categories to three: rarely (1, 2, 3), moderately (4, 5), often (6, 7):
