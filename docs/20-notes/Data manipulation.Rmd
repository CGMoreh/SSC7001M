...
```{r}
evs %>% select("v31":"v37", "v225", "v234", "v227", "age", "v102", "v105")
```
...
To actually collapse the loaded dataset to the selected variables, we have to assign the 'select()' procedure to either a new object name (e.g. smalldata <- ) or the same name (evs <- ), which would overwrite the existing dataset.
...
```{r}
evs %>% count(as_factor(no_trust))
```

We can recode values explicitly this way:

```{r}
evs1 <- evs %>% mutate(no_trust=recode(no_trust,
                                         `1`=0, 
                                         `2`=1))
```
