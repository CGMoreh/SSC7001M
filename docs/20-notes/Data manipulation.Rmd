...
We now have more intuitive variable names, but as we transform/recode our variables it may become useful to rename some of them again later to better reflect their actual meaning. Also, note that we haven't renamed all the variables - <tt>v33</tt>, <tt>v35</tt>, <tt>v36</tt> and <tt>v37</tt> remain unchanged - and that's for a reason, as we'll see in a moment.
...
# Setting 'as.tag = TRUE' let's us store the lables of the missing values, just in case we'll need them again
 
evs.slim.gb <- evs.slim.gb %>% set_na(ppl.trust.yn, na = c(-1:-10), as.tag = TRUE)
```
...
evs.slim.gb <- evs.slim.gb %>% set_na(na = c(-1:-99), as.tag = TRUE)
```

## I need to recode variables

```{r}
evs.slim.gb %>% recode_to(ppl.trust.yn, suffix = "") %>% frq(ppl.trust.yn)
```


We can recode the values themselves:
