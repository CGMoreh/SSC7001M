...
knitr::opts_chunk$set(fig.width=6, fig.asp = 0.618, fig.show = 'hold', warning=FALSE, comment=NA, prompt=FALSE, message = F, strip.white = F, class.source = "bc-b", attr.source='.numberLines', results = 'hold')
```
...
Let's follow such a hypothetical workflow to cover some of these most common requirements. Our hypothetical data analysis aim will be to estimate and report on a model predicting <tt>*trust in strangers*</tt> from <tt>*age*</tt>, <tt>*sex*</tt>, <tt>*religiosity*</tt>, and <tt>*political ideology*</tt> among the UK population:
...
The result, compared tot he original variables, would be:
```{r echo=FALSE}
...
There are many ways to go about this and it is ultimately a theoretical decision. For our purposes, we'll simply create a variable that sums up the values on the five variables we want to use to measure <tt>*trust in strangers*</tt>; to give more weight to the general dichotomous social trust variable (<tt>*Trusting</tt>), we'll make it's value 1 equal to the highest value of the other component variables. Let's do the following operations:
  1. Let's shift the coding range of all component variables to start from 0 (as in the previous step); the scales will then be 0 to 3;
  2. Let's use only observations that have missing response (NA) on fewer than three out of the five variables;
  3. Let's create a new variable called "trust_in_strangers" by:
    3.1. multiplying the value of <tt>*Trusting*</tt> by 3 (0/1 will become 0/3);
    3.2. then adding up the values of the five variables

```{r eval=FALSE, include=FALSE}
evs.slim.gb <- evs.slim.gb %>% 
  sjmisc::recode_to(v33:v37, lowest = 0, suffix = "") %>%             # 1 #
  filter(rowSums(across(c(Trusting, v33:v37), ~ is.na(.))) < 3) %>%   # 2 #
  mutate(trust_in_strangers = (Trusting * 3) + v33 + v35 + v36 + v37) # 3 #
```  

## I need to set the variable types

We can also have a final look at our variables and check if the variable types are recorded correctly. If not, we can set the appropriate types (e.g. our two religiosity variables should be factors):

```{r}
evs.slim.gb <-  evs.slim.gb %>% 
  as_factor(rel.now, rel.as.child)
```


## All ready for analysis

The data is now ready to undertake the regression that we planned:
