...
Let's do the following operations in one pipeline:
  1. Shift the coding range of all component variables to start from 0 (as in the previous step); the scales will then be 0 to 3;
...
Step (2) is a bit convoluted, using the `rowSums` bas ***R*** function to count the missing values (is.na) `across()` the specified variables, and `filter()` out observations where this count is higher than or equal to 3.
...
get_label(evs.slim.gb$trust_in_strangers)
get_labels(evs.slim.gb$trust_in_strangers)

evs.slim.gb2 <- evs.slim.gb %>%
  mutate(trust_in_strangers = sjlabelled::remove_all_labels(trust_in_strangers))
```


## I need to set the variable types

We can also have a final look at our variables and check if the variable types are recorded correctly. If not, we can set the appropriate types (e.g. our two religiosity variables should be factors (i.e. categorical variables)):
