...
Below is a recommended structure and some example code that seems to work well with Word documents.
...
Somewhere around here you may want to show a summary of your variables of interest. For example, a table such as this (using the `descr` function from the `summarytools` package):
...
## Take a dataset
ost %>% 
  ## Treat variables as numeric for the purpose of the table
  as_numeric() %>%
  ## Optionally, select the variables to include in the table; add variable names inside the (), using quotation marks and separated by commas
  select("trustindex3", "ppltrst", "pplfair", "cntry", "facntr", "mocntr", "female", "agea", "eduyrs25", "paredu_a_high") %>%  
  ## The command for the table; no changes necessary here
...
Here is an example of a summary table of a single model using the `parameters` and `insight`packages):
...
A summary table of several models (using the `parameters` and `insight`packages):
...
A graph of one or more models:

```{r}

jtools::plot_summs(M1, M2, M3)
```
